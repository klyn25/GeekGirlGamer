...
<div id="leftcol">
		...
		$q = "SELECT p.*, g.username FROM gggposts AS p INNER JOIN ggger AS g USING (gggerID) WHERE categoryID = 2 ORDER BY $sort";
				...
				echo '<div id="item">
					<h3>' . $row['title'] . '</h3>' .
						'<img src="' . $row['banner'] . '" width="100%" alt="' . $row['title'] . ' image">
						 ...
						 <p class="itemlink"><a href="girls.php?postID=' . $row['postID'] . '">Read more&hellip;</a></p>
					</div>';
			}
